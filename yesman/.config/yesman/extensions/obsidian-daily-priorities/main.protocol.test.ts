...
Deno.test("launcher dispatches one durable daily conversation per workspace/date", async () => {
  ...
  if (
    ...
    firstCreate.method !== "conversations.create"
  ) {
    throw new Error(
  ...
  try {
    await write(writer, {
      ...
      params: {
        ...
        extension_id: "obsidian-daily-priorities",
        extension_release: "0.1.0",
        enabled_workspace_ids: [workspaceId],
        config: { enabled: true, workspace_id: workspaceId, daily_hour: 0 },
      },
    });
    await write(writer, {
      jsonrpc: "2.0",
